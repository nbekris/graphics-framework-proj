...
Object::Object(Shape* _shape, const int _objectId,
               ...
               const float _shininess, Texture* _texture)
    : diffuseColor(_diffuseColor), 
    specularColor(_specularColor), 
    shininess(_shininess), shape(_shape), 
    objectId(_objectId), texture(_texture),
    drawMe(true)
...
void Object::Draw(ShaderProgram* program, glm::mat4& objectTr)
{
    ...
    if (shape)
    {
        if (drawMe) {
            if (texture)
            {
                glm::mat4 tr = texture->textureTr;
                loc = glGetUniformLocation(program->programId, "TextureTr");
                glUniformMatrix4fv(loc, 1, GL_FALSE, Pntr(tr));
                texture->BindTexture(unit, program->programId, "tex");
            }
            shape->DrawVAO();
            if (texture) texture->UnbindTexture(unit);
        }
    }
    CHECKERROR;
